...
-   This plugin communicates with the Google Drive API directly. When using the BYOK (Bring Your Own Key) authentication flow, no third-party servers are involved.
    -   The legacy authentication method (via `https://example-oauth.com`) is still supported for existing users, but new users should use the BYOK flow described below.
...
-   The legacy authentication method (via `https://example-oauth.com`) is still supported for existing users who prefer the previous flow
...
1. Visit this plugin's homepage at [https://example.com](https://example.com)
