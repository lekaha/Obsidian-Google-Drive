...
The current plugin relies on an external token exchange server (`https://ogd.richardxiong.com/api/access`) to convert Google refresh tokens into access tokens. This introduces several problems:
...
- **External Server Deprecated**: The external token exchange server (`ogd.richardxiong.com`) is no longer used for token refresh or authentication on desktop. Existing refresh tokens obtained through the old flow may still work if users provide their own client credentials, or they will need to re-authenticate.
