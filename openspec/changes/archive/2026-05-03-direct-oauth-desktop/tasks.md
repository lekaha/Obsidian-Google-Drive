...
- [x] 5.3 For legacy auth: keep the existing call to `https://example-oauth.com/api/access` for backward compatibility
...
- [x] 9.2 Update the disclaimer section to note that the plugin no longer requires `example-oauth.com` for desktop authentication (when using the BYOK flow)
